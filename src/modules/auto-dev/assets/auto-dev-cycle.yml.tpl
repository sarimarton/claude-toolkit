    ...
    steps:
      ...
      - name: "new → evaluate with Claude"
        ...
        run: |
...

          {
            ...
            echo "- For EPIC: also output 'architecture_overview': a 200-300 word technical overview of the project's architecture, key components, data flows, and module structure. This will be saved as ARCHITECTURE.md for future context."
            ...
            echo ""
            echo 'If EPIC:    {"decision": "epic", "comment": "Why this is an epic", "architecture_overview": "200-300 word technical overview", "lhf": [{"title": "Short task title", "body": "Task description with context"}]}'
      ...
      - name: "new → handle epic"
        ...
        run: |
          ...
          ARCH_OVERVIEW=$(jq -r '.structured_output.architecture_overview // ""' "$WORK_DIR/evaluate-result.json" 2>/dev/null)

          if [[ -n "$ARCH_OVERVIEW" && ! -f "ARCHITECTURE.md" ]]; then
            printf '%s\n' "$ARCH_OVERVIEW" > ARCHITECTURE.md
            git add ARCHITECTURE.md
            git commit -m "docs: add ARCHITECTURE.md [auto-dev]"
            git push
          fi
      ...
      - name: "in-progress → find and execute next todo"
        ...
        run: |
          ...
          if [[ "$STATUS" == "completed" ]]; then
            ...
            UPDATED_BODY=$(echo "$PR_BODY" | awk -v todo="$NEXT_TODO" 'BEGIN{done=0} !done && $0 == "- [ ] " todo {sub(/\[ \]/, "[x]"); done=1} 1')
